...
import 'package:Trackery/animatedloading.dart';
...
class ChooseSi extends StatelessWidget {
  final String data;
  final String type;
  const ChooseSi({Key key, @required this.data, @required this.type})
      : super(key: key);

  Future<List<String>> searchSi() async {
    List<String> silist = [];
    if (type == "sno") {
      var url =
  ...
  _ChooseSi createState() => _ChooseSi();
}

class _ChooseSi extends State {
  Widget build(BuildContext context) {
    return Scaffold();
  }
}
